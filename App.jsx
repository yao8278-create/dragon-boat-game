...
class SynthEngine {
    ...
    playStep(time) {
        ...
        if (this.mode === 'menu' || this.mode === 'shop' || this.mode === 'login') {
...
// 🎨 載入靜態圖片資源 (取代 AI 生成)
// ==========================================
// 圖片檔案必須與此檔案位於同一層目錄 (部署於 Vercel / GitHub Pages)
const ASSET_FILES = {
    boat1_side: 'boat1_side.png', boat1_top: 'boat1_top.png',
    boat2_side: 'boat2_side.png', boat2_top: 'boat2_top.png',
    boat3_side: 'boat3_side.png', boat3_top: 'boat3_top.png',
    boat4_side: 'boat4_side.png', boat4_top: 'boat4_top.png',
    boat5_side: 'boat5_side.png', boat5_top: 'boat5_top.png',
    dragon: 'dragon.png',
    fish: 'fish.png',
...
const loadImages = async () => {
    const loadedAssets = {};
    const promises = Object.entries(ASSET_FILES).map(([key, src]) => {
        return new Promise((resolve, reject) => {
            const img = new Image();
            img.onload = () => { loadedAssets[key] = img; resolve(); };
            img.onerror = () => { console.warn(`Failed to load image: ${src}`); resolve(); }; // 即使失敗也繼續，使用回退圖形
            img.src = src;
        });
    });
...
export default function App() {
  const canvasRef = useRef(null);
  const [currentView, setCurrentView] = useState('login'); // 預設視圖改為 login
  ...
  // 🌟 登入與玩家資料狀態
  const [playerName, setPlayerName] = useState(() => safeGetStorage('local_player_name', ''));
  ...
  // 🌟 Firebase 匿名登入與資料綁定 (依賴 playerName)
  useEffect(() => {
      if (!auth || !playerName) return;
      const initAuth = async () => {
          try { await signInAnonymously(auth); } catch (e) { console.error("Auth failed", e); setIsDataLoaded(true); }
      };
      initAuth();
      const unsubscribe = onAuthStateChanged(auth, (user) => {
          if (user && db) {
              // 🌟 使用 playerName 作為 Document ID 儲存資料
              const userRef = doc(db, 'players', playerName);
              const unsubSnap = onSnapshot(userRef, (docSnap) => {
                  if (docSnap.exists()) {
                      ...
                      // 第一次登入，初始化資料
                      setCoins(0); setUpgrades({ lives: 1, fever: 1 }); setMaxSummons(0);
                  }
                  setIsDataLoaded(true); 
              }, (error) => { console.error("DB error", error); setIsDataLoaded(true); });
              return () => unsubSnap();
  ...
  // 🌟 儲存資料到 Firebase
  useEffect(() => {
      if (!isDataLoaded || !playerName || !db) return; 
      const saveData = async () => {
          try { const userRef = doc(db, 'players', playerName); await setDoc(userRef, { coins, upgrades, maxSummons }, { merge: true }); } catch(e) { console.error("Save failed", e); }
      };
      saveData();
  }, [coins, upgrades, maxSummons, isDataLoaded, playerName]);
  ...
  const handleLogin = (e) => {
      ...
      if (name) {
          setPlayerName(name);
          safeSetStorage('local_player_name', name);
          setCurrentView('loading'); // 登入後進入載入畫面
      }
  };
  ...
  useEffect(() => {
      if (currentView === 'login' || currentView === 'menu' || currentView === 'shop') audio.setMode('menu');
  ...
  useEffect(() => {
      ...
      const initAssets = async () => {
          ...
          try {
              const loadedAssets = await loadImages();
              if(isMounted) {
    ...
    return (
        <div className="flex flex-col items-center justify-center h-screen overflow-hidden bg-blue-50 p-4 relative">
            ...
            <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-[400px] text-center flex flex-col items-center">
                ...
                <h1 className="text-4xl font-black text-blue-900 mb-2">端午龍舟長征</h1>
                <p className="text-gray-500 mb-6 font-bold">請輸入您的名字以記錄榮耀</p>
                <form onSubmit={handleLogin} className="w-full flex flex-col gap-4">
                    <input 
                        ...
                        placeholder="輸入您的名字" 
                        className="w-full p-4 border-2 border-blue-200 rounded-xl text-xl text-center focus:outline-none focus:border-blue-500 font-bold"
                        required
                    />
                    <button type="submit" className="w-full py-4 bg-green-500 hover:bg-green-600 text-white text-xl font-bold rounded-xl shadow-md transform transition-transform hover:scale-105">
                        登入並準備出航
                    </button>
                </form>
            </div>
  ...
  if (currentView === 'loading') { return ( <div className="flex flex-col items-center justify-center h-screen overflow-hidden bg-gray-900 text-white p-6 text-center"> <div className="text-8xl mb-6 animate-bounce">✨</div> <h1 className="text-3xl font-black mb-4 text-blue-300">極速載入中...</h1> <p className="text-gray-400 mb-8 max-w-md">正在為您準備高畫質遊戲素材，請稍候！</p> <div className="w-full max-w-sm h-4 bg-gray-800 rounded-full overflow-hidden mb-2"><div className="h-full bg-gradient-to-r from-blue-500 to-purple-500 transition-all duration-300" style={{ width: `${loadingProgress}%` }}></div></div> <p className="text-purple-300 font-bold">載入靜態資源中...</p> </div> ); }

  if (currentView === 'menu') { return ( <div className="flex flex-col items-center justify-center h-screen overflow-hidden bg-blue-50 p-4 relative"> <SoundToggleButton /> <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-[400px] text-center flex flex-col items-center"> <div className="text-6xl mb-4">🐉🛶</div> <h1 className="text-4xl font-black text-blue-900 mb-2">端午龍舟長征</h1> <p className="text-gray-500 mb-6">歡迎回來，<span className="font-bold text-blue-600">{playerName}</span>！</p> <div className="flex gap-3 mb-8 w-full justify-center"> <div className="bg-yellow-100 px-4 py-2 rounded-full font-bold text-yellow-700 text-sm shadow-sm">💰 資金: {coins}</div> <div className="bg-blue-100 px-4 py-2 rounded-full font-bold text-blue-700 text-sm shadow-sm">🐉 最高召喚: {maxSummons} 次</div> </div> <div className="flex flex-col gap-4 w-full"> <button onClick={startGame} className="w-full py-4 bg-green-500 hover:bg-green-600 text-white text-xl font-bold rounded-xl shadow-md transform transition-transform hover:scale-105">▶ 開始長征</button> <button onClick={() => setCurrentView('shop')} className="w-full py-4 bg-blue-500 hover:bg-blue-600 text-white text-xl font-bold rounded-xl shadow-md transform transition-transform hover:scale-105">🛠️ 龍舟改造廠</button> </div> </div> </div> ); }
